...
export function Panel({
  ...
  return (
    <section className={cn("panel flex min-w-0 flex-col", className)}>
      {(title || right) && (
        <header className="flex items-start justify-between gap-4 border-b border-line px-4 py-3">
          <div className="min-w-0">
            {title && <h2 className="label text-purple">{title}</h2>}
            {sub && <p className="mt-1 text-xs leading-relaxed text-grey">{sub}</p>}
          </div>
      ...
      {footnote && (
        <footer className="border-t border-line bg-purple-lt/40 px-4 py-2.5 text-xs leading-relaxed text-grey">
          {footnote}
        </footer>
      )}
    </section>
...
export function Field({
  ...
  return (
    <div className="min-w-0">
      ...
      <p className={cn("mt-0.5 truncate text-sm text-ink", mono && "machine")}>{children}</p>
    </div>
  );
}
