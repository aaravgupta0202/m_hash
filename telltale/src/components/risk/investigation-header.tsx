...
const TONE_TEXT = { green: "text-emerald-700", amber: "text-amber-700", red: "text-red-700" } as const;
...
export function InvestigationHeader({ subject, dossier }: { subject: Subject; dossier: Dossier }) {
  ...
  return (
    <div className="mb-4">
      ...
      <div className="panel flex flex-col lg:flex-row items-stretch border border-line bg-white rounded-lg overflow-hidden shadow-xs">
        ...
        <div className="flex w-full lg:w-60 shrink-0 flex-col justify-center border-b lg:border-b-0 lg:border-r border-line bg-slate-50/50 px-5 py-4">
          <p className="label text-slate-500 text-[10px]">Calibrated risk score</p>
          <div className="mt-1 flex items-baseline gap-2">
            <span className={cn("machine text-5xl leading-none font-extrabold tracking-tight", TONE_TEXT[tone])}>
              {dossier.risk}
            </span>
            <span className="machine text-sm text-slate-400 font-normal">/100</span>
          </div>
        ...
        <div className="flex min-w-0 flex-1 flex-col justify-between gap-3 px-5 py-4">
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <h1 className="machine text-2xl leading-none font-bold text-slate-900 tracking-tight">{subject.pseudonym}</h1>
            ...
            <p className="mt-2.5 max-w-3xl text-sm leading-relaxed text-slate-700">{dossier.headline}</p>
          </div>
        ...
        <div className="flex w-full lg:w-56 shrink-0 flex-col justify-between border-t lg:border-t-0 lg:border-l border-line bg-slate-50/40 px-4 py-4">
          ...
          <p className="mt-2 text-[10px] leading-relaxed text-slate-500">
            Each action explains simulated workflow. No live connectors called in demo.
          </p>
        </div>
      </div>
    </div>
