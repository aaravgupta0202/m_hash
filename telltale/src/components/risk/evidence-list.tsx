    ...
    <div>
      <div className="flex items-center justify-between gap-4 border-b border-line px-4 py-2 bg-slate-50/50">
        <p className="machine text-xs text-slate-500">
          {items.length} artefacts · {flaggedCount} flagged · {taggedCount}{" "}
          tagged
        </p>
        <p className="text-[11px] text-slate-400">
          Tags and flags are session state in this build and reset on reload.
        </p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="machine text-xs leading-relaxed text-slate-800">
                  {item.detail}
                </p>
                <div className="mt-1 flex flex-wrap items-center gap-1.5">
                  <span className="flex items-center gap-1 text-[11px] text-slate-400">
                    <BrandIcon name={item.source} className="size-3" />
                    {item.source}
                  </span>
                  {itemTags.map((t) => (
                    <span
                      key={t}
                      className="label rounded border border-emerald-200 bg-emerald-50 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-800"
                    >
                      {t}
                    </span>
                  ))}
              ...
              <div className="flex shrink-0 items-center gap-1">
                <Popover>
                  ...
                  <PopoverContent
                    align="end"
                    className="w-56 rounded-sm p-1 shadow-lg bg-white border border-line text-slate-800 z-50"
                  >
                    {TAGS.map((t) => (
                      <button
                        key={t}
                ...
                <Popover>
                  ...
                  <PopoverContent
                    align="end"
                    className="w-72 rounded-sm p-2 shadow-lg bg-white border border-line text-slate-800 z-50"
                  >
                    <p className="label px-2 py-1 text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                      Reason code
                    </p>
                    {REASON_CODES.map((r) => (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          setFlags((prev) => ({ ...prev, [item.id]: r.code }))
                        }
                        className="block w-full rounded px-2 py-1.5 text-left hover:bg-slate-50"
                      >
                        <span className="machine block text-xs font-bold text-slate-900">
                          {r.code}
                        </span>
