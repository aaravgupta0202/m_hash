...
export function CohortStrip({ comparison }: { comparison: CohortComparison }) {
  ...
  const domainMax = broken
    ? peerMax * 1.08
    : Math.max(subjectValue, peerMax) * 1.08;
        ...
        <svg viewBox={`0 0 ${W} ${H}`} width={W} height={H} className="block">
          ...
          <g>
            ...
            <rect
              ...
              height="10"
              fill="var(--tt-red)"
              transform={`rotate(45 ${subjectX} ${DOT_Y})`}
            >
          ...
          <text
            ...
            fill="var(--tt-grey)"
          >
            n = {stats.n} peers · median {stats.median} · MAD {stats.mad} ·
            robust Z ={" "}
            <tspan
              fill={Math.abs(zScore) > 3 ? "var(--tt-red)" : "var(--tt-ink)"}
              fontWeight="600"
            >
              {zScore >= 0 ? "+" : "−"}
...
function pack(values: number[], x: (v: number) => number) {
  ...
  for (const v of values) {
    const cx = x(v);
    let row = 0;
    while (placed.some((p) => p.row === row && Math.abs(p.cx - cx) < 7.2))
      row += 1;
    placed.push({ cx, row });
  }
  return placed;
